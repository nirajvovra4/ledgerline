...
describe('auth schemas', () => {
  it('registerSchema normalises the email and enforces password length', () => {
    expect(
      ok(registerSchema, { name: ' Ada ', email: ' Ada@Example.COM ', password: 'password123' }),
    ).toEqual({
      name: 'Ada',
    ...
    expect(bad(registerSchema, { name: 'Ada', email: 'a@b.co', password: 'short' })).toEqual({
    ...
    expect(bad(registerSchema, { email: 'a@b.co', password: 'password123' }).name).toBe(
      'Name is required',
    );
    expect(
      bad(registerSchema, { name: 'x'.repeat(81), email: 'a@b.co', password: 'password123' }).name,
    ).toBe('Name must be 80 characters or fewer');
  });

  it('loginSchema requires a password', () => {
    expect(ok(loginSchema, { email: 'A@B.CO', password: 'x' })).toEqual({
      email: 'a@b.co',
      password: 'x',
    });
    expect(bad(loginSchema, { email: 'a@b.co', password: '' })).toEqual({
      password: 'Enter your password',
    });
  });

  it('profile and password change', () => {
    expect(ok(updateProfileSchema, { name: 'Ada', email: 'a@b.co' })).toEqual({
      name: 'Ada',
      email: 'a@b.co',
    });
    expect(
...
describe('workspace schemas', () => {
  it('accepts the default settings', () => {
    expect(ok(workspaceSettingsSchema, DEFAULT_WORKSPACE_SETTINGS)).toEqual(
      DEFAULT_WORKSPACE_SETTINGS,
    );
    expect(
      ok(workspaceSettingsSchema, {
        ...DEFAULT_WORKSPACE_SETTINGS,
        defaultTaxRateId: U1,
        email: 'Ops@Studio.io',
      }),
  ...
  it('rejects out-of-range settings with field-level messages', () => {
    const settings = (patch: Partial<typeof DEFAULT_WORKSPACE_SETTINGS>) =>
      bad(workspaceSettingsSchema, { ...DEFAULT_WORKSPACE_SETTINGS, ...patch });
    expect(settings({ invoicePrefix: 'IN V' })).toEqual({
      invoicePrefix: 'Letters and numbers only',
    });
    expect(settings({ invoicePrefix: 'ABCDEFGHI' }).invoicePrefix).toBe(
      'Prefix must be 8 characters or fewer',
    );
    ...
    expect(settings({ defaultPaymentTermsDays: 366 })).toEqual({
      defaultPaymentTermsDays: 'At most 365 days',
    });
    expect(settings({ defaultPaymentTermsDays: -1 })).toEqual({
      defaultPaymentTermsDays: 'Cannot be negative',
    });
  ...
  it('createWorkspaceSchema validates currency and slug', () => {
    expect(ok(createWorkspaceSchema, { name: 'Northlight', currency: 'USD' })).toEqual({
      name: 'Northlight',
      currency: 'USD',
    });
    expect(
      ok(createWorkspaceSchema, { name: 'Northlight', currency: 'EUR', slug: ' My-Studio ' }).slug,
    ).toBe('my-studio');
    expect(
      bad(createWorkspaceSchema, { name: 'Northlight', currency: 'XXX' }).currency,
    ).toBeDefined();
    expect(
      bad(createWorkspaceSchema, { name: 'Northlight', currency: 'USD', slug: 'my studio' }),
    ).toEqual({
      slug: 'Use lowercase letters, numbers and hyphens',
    });
    expect(
      bad(createWorkspaceSchema, { name: 'Northlight', currency: 'USD', slug: '-bad-' }).slug,
    ).toBe('Use lowercase letters, numbers and hyphens');
    expect(
      bad(createWorkspaceSchema, { name: 'Northlight', currency: 'USD', slug: 'a' }).slug,
    ).toBe('Slug must be at least 2 characters');
    expect(bad(createWorkspaceSchema, { name: '', currency: 'USD' })).toEqual({
  ...
  it('updateWorkspaceSchema accepts partial settings', () => {
    ...
    expect(ok(updateWorkspaceSchema, { settings: { requireInvoiceApproval: false } })).toEqual({
      settings: { requireInvoiceApproval: false },
    });
    expect(
      bad(updateWorkspaceSchema, { settings: { nextInvoiceNumber: 0 } })[
  ...
  it('member schemas', () => {
    expect(ok(inviteMemberSchema, { email: 'X@Y.io', role: 'admin' })).toEqual({
      email: 'x@y.io',
      role: 'admin',
    });
...
describe('list & range queries', () => {
  it('listQuerySchema coerces page and pageSize from strings with fallbacks', () => {
    ...
    expect(ok(listQuerySchema, { page: '3', pageSize: '50' })).toMatchObject({
      page: 3,
      pageSize: 50,
    });
    expect(ok(listQuerySchema, { page: 2, pageSize: 200 })).toMatchObject({
      page: 2,
      pageSize: 200,
    });
    expect(ok(listQuerySchema, { page: 'abc', pageSize: 'xyz' })).toMatchObject({
      page: 1,
      pageSize: 25,
    });
    expect(ok(listQuerySchema, { page: '0', pageSize: '0' })).toMatchObject({
      page: 1,
      pageSize: 25,
    });
    expect(ok(listQuerySchema, { page: '', pageSize: '' })).toMatchObject({
      page: 1,
      pageSize: 25,
    });
  ...
  it('listQuerySchema trims q and validates dir', () => {
    expect(ok(listQuerySchema, { q: '  acme ', sort: 'name', dir: 'desc' })).toMatchObject({
      q: 'acme',
      sort: 'name',
      dir: 'desc',
    });
  ...
  it('status filters fall back on unknown values', () => {
    ...
    expect(
      ok(invoiceListQuerySchema, { status: 'overdue', clientId: U1, from: '2024-01-01' }),
    ).toMatchObject({ status: 'overdue', clientId: U1 });
  ...
  it('coerces booleans from query strings', () => {
    expect(ok(timeEntryListQuerySchema, { billable: 'true', uninvoiced: '1' })).toMatchObject({
      billable: true,
      uninvoiced: true,
    });
    expect(ok(timeEntryListQuerySchema, { billable: 'false', uninvoiced: '0' })).toMatchObject({
      billable: false,
      uninvoiced: false,
    });
  ...
  it('dateRangeSchema requires from <= to', () => {
    ...
    expect(ok(dateRangeSchema, { from: '2024-01-01', to: '2024-01-01' })).toEqual({
      from: '2024-01-01',
      to: '2024-01-01',
    });
    expect(ok(dateRangeSchema, { from: '2024-01-01' })).toEqual({ from: '2024-01-01' });
    expect(bad(dateRangeSchema, { from: '2024-02-01', to: '2024-01-01' })).toEqual({
      to: '"From" must be before "to"',
    });
  ...
  it('profit/loss query combines range with compare', () => {
    expect(
      ok(profitLossQuerySchema, { from: '2024-01-01', to: '2024-03-31', compare: 'previous' }),
    ).toEqual({
      from: '2024-01-01',
      to: '2024-03-31',
      compare: 'previous',
  ...
  it('other list queries validate their filters', () => {
    expect(ok(journalListQuerySchema, { sourceType: 'reversal', accountId: U1 })).toMatchObject({
      sourceType: 'reversal',
      accountId: U1,
    });
...
describe('clientInputSchema', () => {
  ...
  it('rejects bad values', () => {
    ...
    expect(bad(clientInputSchema, { name: 'A', paymentTermsDays: 400 })).toEqual({
      paymentTermsDays: 'At most 365 days',
    });
    expect(bad(clientInputSchema, { name: 'A', paymentTermsDays: -1 }).paymentTermsDays).toBe(
      'Cannot be negative',
    );
    ...
    expect(bad(clientInputSchema, { name: 'A', notes: 'x'.repeat(2001) }).notes).toBe(
...
describe('projectInputSchema', () => {
  ...
  it('upper-cases the code and validates its characters', () => {
    ...
    expect(bad(projectInputSchema, { ...base, code: 'web 1' })).toEqual({
      code: 'Letters, numbers and hyphens only',
    });
    expect(bad(projectInputSchema, { ...base, code: 'ABCDEFGHIJKLM' }).code).toBe(
  ...
  it('enforces the date ordering refinement', () => {
    expect(
      ok(projectInputSchema, { ...base, startDate: '2024-01-01', endDate: '2024-01-01' }),
    ).toMatchObject({ startDate: '2024-01-01', endDate: '2024-01-01' });
    expect(ok(projectInputSchema, { ...base, startDate: '', endDate: '2024-01-01' })).toMatchObject(
      { startDate: null, endDate: '2024-01-01' },
    );
    expect(
      bad(projectInputSchema, { ...base, startDate: '2024-02-01', endDate: '2024-01-01' }),
    ).toEqual({ endDate: 'End date must be after the start date' });
    expect(bad(projectInputSchema, { ...base, startDate: '2024-02-30' }).startDate).toBe(
      'Enter a valid date (YYYY-MM-DD)',
    );
  });

  it('requires whole non-negative cents', () => {
    expect(bad(projectInputSchema, { ...base, hourlyRateCents: -1 })).toEqual({
      hourlyRateCents: 'Amount cannot be negative',
    });
    expect(bad(projectInputSchema, { ...base, budgetCents: 10.5 })).toEqual({
      budgetCents: 'Amounts must be whole cents',
    });
    expect(bad(projectInputSchema, { ...base, clientId: 'nope' })).toEqual({
      clientId: 'Invalid id',
    });
...
describe('timeEntryInputSchema', () => {
  ...
  it('accepts a valid entry with defaults', () => {
    expect(ok(timeEntryInputSchema, base)).toEqual({
      projectId: U1,
      date: '2024-06-03',
      minutes: 30,
      description: '',
    ...
    expect(
      ok(timeEntryInputSchema, { ...base, userId: U2, billable: false, description: ' work ' }),
    ).toMatchObject({ userId: U2, billable: false, description: 'work' });
  });
  ...
  it('bounds minutes to 1..1440 whole minutes', () => {
    ...
    expect(bad(timeEntryInputSchema, { ...base, minutes: 0 })).toEqual({
      minutes: 'Log at least one minute',
    });
    expect(bad(timeEntryInputSchema, { ...base, minutes: 1441 })).toEqual({
      minutes: 'A single entry cannot exceed 24 hours',
    });
    expect(bad(timeEntryInputSchema, { ...base, minutes: 30.5 }).minutes).toBeDefined();
    expect(bad(timeEntryInputSchema, { ...base, date: '2024-13-01' }).date).toBe(
      'Enter a valid date (YYYY-MM-DD)',
    );
  });
});
...
describe('taxRateInputSchema', () => {
  it('validates the rate and defaults isDefault', () => {
    expect(ok(taxRateInputSchema, { name: 'VAT', rateBp: 2000 })).toEqual({
      name: 'VAT',
      rateBp: 2000,
      isDefault: false,
    });
    expect(bad(taxRateInputSchema, { name: 'VAT', rateBp: 10001 })).toEqual({
      rateBp: 'Cannot exceed 100%',
    });
...
describe('invoiceInputSchema', () => {
  ...
  it('accepts a valid invoice and fills defaults', () => {
    ...
    expect(
      ok(invoiceInputSchema, {
        ...base,
        projectId: U3,
        discountBp: 1000,
        lines: [{ ...line, id: U3, taxRateId: U1 }],
      }),
    ).toMatchObject({
      projectId: U3,
      discountBp: 1000,
  ...
  it('requires at least one line and at most 200', () => {
    expect(bad(invoiceInputSchema, { ...base, lines: [] })).toEqual({
      lines: 'Add at least one line',
    });
    expect(
      bad(invoiceInputSchema, { ...base, lines: Array.from({ length: 201 }, () => line) }).lines,
    ).toBeDefined();
  });

  it('rejects a due date before the issue date but allows the same day', () => {
    expect(bad(invoiceInputSchema, { ...base, dueDate: '2024-05-31' })).toEqual({
  ...
  it('validates quantities: positive, at most four decimals, including binary-awkward values', () => {
    ...
    expect(bad(invoiceLineInputSchema, { ...line, quantity: 0 })).toEqual({
      quantity: 'Quantity must be greater than zero',
    });
    expect(bad(invoiceLineInputSchema, { ...line, quantity: -1 }).quantity).toBe(
      'Quantity must be greater than zero',
    );
    expect(bad(invoiceLineInputSchema, { ...line, quantity: 1.23456 })).toEqual({
      quantity: 'At most four decimal places',
    });
    expect(bad(invoiceLineInputSchema, { ...line, quantity: 1.00001 }).quantity).toBe(
      'At most four decimal places',
    );
    expect(bad(invoiceLineInputSchema, { ...line, quantity: 1000001 }).quantity).toBeDefined();
    expect(bad(invoiceLineInputSchema, { ...line, quantity: '2' })).toEqual({
      quantity: 'Enter a quantity',
    });
    expect(
      bad(invoiceInputSchema, { ...base, lines: [{ ...line, quantity: 0 }] })['lines.0.quantity'],
    ).toBe('Quantity must be greater than zero');
  });

  it('validates the remaining line fields', () => {
    expect(bad(invoiceLineInputSchema, { ...line, description: ' ' })).toEqual({
      description: 'Description is required',
    });
    expect(bad(invoiceLineInputSchema, { ...line, unitPriceCents: 10.5 })).toEqual({
      unitPriceCents: 'Amounts must be whole cents',
    });
    ...
    expect(bad(invoiceLineInputSchema, { ...line, accountId: 'x' })).toEqual({
      accountId: 'Invalid id',
    });
    expect(bad(invoiceLineInputSchema, { ...line, taxRateId: 'x' }).taxRateId).toBe('Invalid id');
    expect(bad(invoiceInputSchema, { ...base, discountBp: 10001 }).discountBp).toBe(
      'Cannot exceed 100%',
    );
    expect(bad(invoiceInputSchema, { ...base, poNumber: 'x'.repeat(61) }).poNumber).toBe(
      'PO number must be 60 characters or fewer',
    );
  });

  it('invoiceFromTimeSchema needs entries and defaults groupBy', () => {
    expect(ok(invoiceFromTimeSchema, { clientId: U1, entryIds: [U2] })).toEqual({
      clientId: U1,
      projectId: null,
      entryIds: [U2],
      groupBy: 'entry',
    });
    expect(bad(invoiceFromTimeSchema, { clientId: U1, entryIds: [] })).toEqual({
      entryIds: 'Select at least one time entry',
    });
    expect(
      bad(invoiceFromTimeSchema, { clientId: U1, entryIds: [U2], groupBy: 'week' }).groupBy,
    ).toBeDefined();
  });
...
describe('recordPaymentSchema', () => {
  ...
  it('accepts a payment and defaults the free-text fields', () => {
    ...
    expect(ok(recordPaymentSchema, { ...base, method: 'cheque', reference: ' 42 ' })).toMatchObject(
      { method: 'cheque', reference: '42' },
    );
  });

  it('requires a positive whole-cent amount and a known method', () => {
    expect(bad(recordPaymentSchema, { ...base, amountCents: 0 })).toEqual({
      amountCents: 'Amount must be greater than zero',
    });
    expect(bad(recordPaymentSchema, { ...base, amountCents: -5 }).amountCents).toBe(
      'Amount must be greater than zero',
    );
    expect(bad(recordPaymentSchema, { ...base, amountCents: 10.5 }).amountCents).toBe(
      'Amounts must be whole cents',
    );
    expect(bad(recordPaymentSchema, { ...base, amountCents: '50' }).amountCents).toBe(
      'Enter an amount',
    );
  ...
  it('payExpenseSchema is the same shape without an amount', () => {
    expect(ok(payExpenseSchema, { date: '2024-06-15', method: 'card' })).toEqual({
      date: '2024-06-15',
      method: 'card',
      reference: '',
    });
...
describe('expenseInputSchema', () => {
  ...
  it('accepts a valid expense with defaults', () => {
    ...
    expect(ok(expenseInputSchema, { ...base, dueDate: '', taxRateId: U2 })).toMatchObject({
      dueDate: null,
      taxRateId: U2,
    });
  });
  ...
  it('requires a client when billable', () => {
    expect(bad(expenseInputSchema, { ...base, billable: true })).toEqual({
      clientId: 'Billable expenses need a client',
    });
    expect(ok(expenseInputSchema, { ...base, billable: true, clientId: U2 })).toMatchObject({
      billable: true,
      clientId: U2,
    });
    expect(ok(expenseInputSchema, { ...base, billable: false, clientId: null }).billable).toBe(
      false,
    );
  });

  it('validates amount and required text', () => {
    expect(bad(expenseInputSchema, { ...base, amountCents: 0 })).toEqual({
      amountCents: 'Amount must be greater than zero',
    });
    expect(bad(expenseInputSchema, { ...base, vendor: '' })).toEqual({
      vendor: 'Vendor is required',
    });
    expect(bad(expenseInputSchema, { ...base, description: ' ' })).toEqual({
      description: 'Description is required',
    });
...
describe('ledger schemas', () => {
  it('accountInputSchema validates the code', () => {
    ...
    expect(bad(accountInputSchema, { code: '12', name: 'AR', type: 'asset' })).toEqual({
      code: 'Code must be 3–6 digits',
    });
    expect(bad(accountInputSchema, { code: '1234567', name: 'AR', type: 'asset' }).code).toBe(
      'Code must be 3–6 digits',
    );
    expect(bad(accountInputSchema, { code: '12a4', name: 'AR', type: 'asset' })).toEqual({
      code: 'Codes are numeric',
    });
    expect(bad(accountInputSchema, { code: '1200', name: '', type: 'asset' })).toEqual({
      name: 'Account name is required',
    });
    expect(
      bad(accountInputSchema, { code: '1200', name: 'AR', type: 'income' }).type,
    ).toBeDefined();
    expect(
      ok(accountInputSchema, {
        code: '1200',
        name: 'AR',
        type: 'asset',
        parentId: U1,
    ...
    ).toMatchObject({ parentId: U1, archived: true });
  });
  ...
  it('manualJournalEntrySchema requires two balanced lines', () => {
    ...
    expect(
      bad(manualJournalEntrySchema, { ...entry, lines: [{ accountId: U1, debitCents: 1000 }] })
        .lines,
    ).toBe('An entry needs at least two lines');
    expect(bad(manualJournalEntrySchema, { ...entry, memo: '' })).toEqual({
  ...
  it('each journal line is either a debit or a credit, never both or neither', () => {
    ...
    expect(
      bad(manualJournalEntrySchema, entry({ accountId: U1, debitCents: 10, creditCents: 10 }))[
        'lines.0.creditCents'
      ],
    ).toBe('A line is either a debit or a credit');
    expect(bad(manualJournalEntrySchema, entry({ accountId: U1 }))['lines.0.debitCents']).toBe(
      'Enter an amount',
    );
    expect(
      bad(manualJournalEntrySchema, entry({ accountId: U1, debitCents: -5 }))['lines.0.debitCents'],
    ).toBe('Amount cannot be negative');
    expect(
      bad(manualJournalEntrySchema, entry({ accountId: 'x', debitCents: 10 }))['lines.0.accountId'],
    ).toBe('Invalid id');
  });

  it('reverseEntrySchema is fully optional', () => {
    expect(ok(reverseEntrySchema, {})).toEqual({ memo: '' });
    expect(ok(reverseEntrySchema, { date: '2024-06-02', memo: ' undo ' })).toEqual({
      date: '2024-06-02',
      memo: 'undo',
    });
