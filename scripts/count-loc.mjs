...
const INCLUDE_EXT = new Set(['.ts', '.tsx', '.css', '.mjs', '.js', '.html', '.sql', '.md', '.yml', '.yaml', '.json']);
const SKIP_DIRS = new Set(['node_modules', 'dist', 'coverage', '.git', 'data', 'screenshots', '.vite']);
...
for (const [key, b] of rows) {
  console.log(`${key.padEnd(width)}  ${String(b.files).padStart(6)}  ${String(b.lines).padStart(8)}`);
}
console.log('-'.repeat(width + 18));
console.log(`${'Total'.padEnd(width)}  ${String(totalFiles).padStart(6)}  ${String(totalLines).padStart(8)}`);
console.log(`${'  of which tests'.padEnd(width)}  ${String(testFiles).padStart(6)}  ${String(testLines).padStart(8)}`);
